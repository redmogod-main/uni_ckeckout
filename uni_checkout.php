...
public function index() {
	...
	private function getShippingMethods() {
		...
		// Отримуємо всі активні методи доставки
		...
		foreach ($results as $result) {
			if ($this->config->get('shipping_' . $result['code'] . '_status')) {
				...
				// Використовуємо адресу доставки з сесії або створюємо базову
				$address = isset($this->session->data['shipping_address']) ? $this->session->data['shipping_address'] : $this->getDefaultAddress();
	...
	private function getDefaultAddress() {
		// Повертаємо базову адресу для розрахунку доставки
		return [
	...
	public function validate() {
		...
		if (!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) {
			$json['error']['error_product'] = $this->language->get('error_product');
		}
		...
		if(!$this->customer->isLogged() && (isset($this->request->post['add-new-customer']) || isset($settings['passgen']))) {
			if ($email && $this->model_account_customer->getTotalCustomersByEmail($email)) {
				$json['error']['email'] = sprintf($this->language->get('error_exists'), $this->url->link('account/login'));
			}
		}
		
		if(isset($settings['telephone']['status'])) {
			if ((isset($settings['telephone']['required']) || $telephone) && (mb_strlen($telephone) < 6 || mb_strlen($telephone) > 25 || strpos($telephone, '_'))) {
		...
		if(isset($settings['pickup']['status']) && !empty($this->session->data['unicheckout_pickup']) && $this->session->data['selected_method'] == $this->method_pickup){
			...
			if (isset($this->request->post['existing-address'])) {
				...
				if(isset($settings['address']['status'])) {
					if ((isset($settings['address']['required']) || $address_1) && (mb_strlen($address_1) < 3 || mb_strlen($address_1) > 128)) {
						...
						if(empty($this->session->data['payment_address']['address_1']) || $this->session->data['payment_address']['address_1'] != $address_1) {
							$this->session->data['payment_address']['address_1'] = $address_1;
						}
						
						if(empty($this->session->data['shipping_address']['address_1']) || $this->session->data['shipping_address']['address_1'] != $address_1) {
				...
				if(isset($settings['address2']['status'])) {
					if ((isset($settings['address2']['required']) || $address_2) && (mb_strlen($address_2) < 3 || mb_strlen($address_2) > 128)) {
						...
						if(empty($this->session->data['payment_address']['address_2']) || $this->session->data['payment_address']['address_2'] != $address_2) {
							$this->session->data['payment_address']['address_2'] = $address_2;
						}
						
						if(empty($this->session->data['shipping_address']['address_2']) || $this->session->data['shipping_address']['address_2'] != $address_2) {
		...
		//shipping method
		if ($this->cart->hasProducts() && $validate_address) {
	...
	public function address() {
		...
		$country_id = !isset($settings['country']['status']) || isset($settings['country']['default_value']) ? $this->config->get('config_country_id') : 0;
		$zone_id = !isset($settings['zone']['status']) || isset($settings['zone']['default_value']) ? $this->config->get('config_zone_id') : 0;
		...
		if(!$this->customer->isLogged() || $new_address) {
			...
			} elseif (!empty($this->session->data['payment_address']['zone_id'])) {
		...
		$data['inputs'] = [
			'city'	=> [
				...
				'value'			=> $city,
				'type'			=> 'text',
				'status' 		=> isset($settings['city']['status']),
				'required' 		=> isset($settings['city']['required']),
				'sort_order'	=> $settings['city']['sort_order']
			],
			'address_1'	=> [
				'placeholder'	=> isset($settings['address']['text'][$lang_id]) ? $settings['address']['text'][$lang_id] : '',
				'value'			=> $address_1,
				'type'			=> 'text',
				'status' 		=> isset($settings['address']['status']),
				'required' 		=> isset($settings['address']['required']),
				'sort_order'	=> $settings['address']['sort_order']
			],
			'address_2'	=> [
				'placeholder'	=> isset($settings['address2']['text'][$lang_id]) ? $settings['address2']['text'][$lang_id] : '',
				'value'			=> $address_2,
				'type'			=> 'text',
				'status' 		=> isset($settings['address2']['status']),
				'required' 		=> isset($settings['address2']['required']),
				'sort_order'	=> $settings['address2']['sort_order']
			],
			'postcode'	=> [
				'placeholder'	=> isset($settings['postcode']['text'][$lang_id]) ? $settings['postcode']['text'][$lang_id] : '',
				'value'			=> $postcode,
				'type'			=> 'tel',
				'status' 		=> isset($settings['postcode']['status']),
				'required' 		=> isset($settings['postcode']['required']),
				'sort_order'	=> $settings['postcode']['sort_order']
			]
		];
		...
		$data['show_country'] = isset($settings['country']['status']);
		$data['show_zone'] = isset($settings['zone']['status']);
	...
	public function setPickupItem() {
		$uniset = $this->config->get('config_unishop2');
		$lang_id = $this->config->get('config_language_id');
		$status = isset($uniset['checkout']['pickup']['status']);
		$pickup_items = [];
	...
	public function shipping_method() { 
		...
		// Зберігаємо всі методи в сесії
		$this->session->data['shipping_methods'] = $method_data; 
		
		// Для шаблону передаємо тільки методи доставки (виключаємо самовивіз)
		$delivery_methods = [];
		foreach ($method_data as $key => $method) {
			if ($key != 'multistore_pickup') {
				$delivery_methods[$key] = $method;
			}
		}
		$data['shipping_methods'] = $delivery_methods;
		...
		if((!isset($this->session->data['shipping_method']) || empty($this->session->data['shipping_method']['code'])) && $method_data) { 
			$first_method = []; 
		...
		$data['text_delivery_methods'] = $this->language->get('text_delivery_methods') ?: 'Способи доставки';
		 
		// Перевіряємо помилки тільки для методів доставки (без самовивозу)
		$data['error_warning'] = (empty($delivery_methods)) ? sprintf($this->language->get('error_no_shipping'), $this->url->link('information/contact')) : ''; 
	...
	private function totals() {
		...
		$data['new_class'] = isset($uniset['header']['is_fixed']['status']) ? 'header-is-fixed' : '';
		...
		$data['coupon'] = [
			'status'	=> $this->config->get('total_coupon_status') ? true : false,
			'value'		=> isset($this->session->data['coupon']) ? $this->session->data['coupon'] : ''
		];
			
		$data['voucher'] = [
			'status'	=> $this->config->get('total_voucher_status') ? true : false,
			'value'		=> isset($this->session->data['voucher']) ? $this->session->data['voucher'] : ''
		];
		...
		$data['reward'] = [
			'status'	=> ($points && $points_total && $this->config->get('total_reward_status')) ? true : false,
			'value'		=> isset($this->session->data['reward']) ? $this->session->data['reward'] : ''
		];
		
		if($data['reward']['status']) {
			$data['reward']['text'] = sprintf($this->language->get('text_reward_placeholder'), $this->getFormat($points), $this->getFormat(($points <= $points_total ? $points : $points_total)));
		}
		
		if(!empty($this->session->data['vouchers'])) {
			$data['coupon']['status'] = $data['voucher']['status'] = $data['reward']['status'] = '';
		}
		...
		if(isset($uniset['checkout']['pickup']['status']) && !empty($this->session->data['unicheckout_pickup']) && isset($this->session->data['selected_method']) && $this->session->data['selected_method'] == $this->method_pickup) {
		...
		$data['agree_checked'] = isset($uniset['checkout']['agree']['checked']) ? true : false;
	...
	private function getFormat($data) {
		$decimals = $this->currency->getDecimalPlace($this->session->data['currency']);
		
		return number_format($data, $decimals, $this->language->get('decimal_point'), $this->language->get('thousand_point'));
	}
